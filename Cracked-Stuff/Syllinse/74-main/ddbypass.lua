...
local sec_patched = 0
--[[ Timer в новом дампе не изменился (class/Timer:29):
        expired() = (next_time < Timescale.get_time())
        reset()   = next_time = get_time() + timeout
     поэтому next_time=huge И timeout=huge => expired() НИКОГДА не истинно,
     даже после reset(). Патч по-прежнему верный.

     ОПТИМИЗАЦИЯ: раньше filtergc гонялся здесь каждые 2 с (фоновый цикл) —
     это обход всей кучи, то есть вклад в просадку FPS. Теперь держим ссылки
     на уже найденные таймеры: пока они пришиты, обход GC не нужен вообще.
     Скан запускается только если пришитых таймеров не осталось (респавн
     пересоздаёт контроллер), и не чаще SEC_RESCAN. ]]
local secTimers  = {}
local lastSecScan = -HUGE
local SEC_RESCAN  = 3.0

local function pin_timer(t)
    if type(t) ~= "table" then return false end
    if rawget(t, "next_time") ~= HUGE or rawget(t, "timeout") ~= HUGE then
...
local function patch_secondary()
    -- быстрый путь: подтверждаем уже пришитые таймеры (без GC)
    local alive = 0
    for i = #secTimers, 1, -1 do
        local t = secTimers[i]
        if type(t) == "table" then
            pin_timer(t)
            alive += 1
        else
            table.remove(secTimers, i)
        end
    end
    if alive > 0 then return end

    local now = os.clock()
    if now - lastSecScan < SEC_RESCAN then return end
    lastSecScan = now
...
----------------------------------------------------------------------
-- L6 — БЛОКИРОВКА FPS-THROTTLE (падение до 1 fps). Найдено в дампе:
--
--   client/controller/misc/limit_fps:11-21
--     while true do
--         repeat v1 = os.clock(); RunService.RenderStepped:Wait()
--         until SHARED_STATE.dbg_limit_fps.value        -- гейт
--         while v1 + 1 / SHARED_STATE.dbg_max_fps.value > os.clock() do
--         end                                          -- BUSY-WAIT, жжёт кадр
--     end
--
--   module/shared_state:469-485  (клиентская ветка)
--     changed.OnClientEvent:Connect(function(p27, p28)
--         u15[p27]:set_client(p28)                     -- СЛЕПО применяет
--     end)
--
-- То есть это НЕ freeze-loop, а наказание: сервер по каналу `changed`
-- присылает dbg_limit_fps=true (и низкий dbg_max_fps), после чего клиент
-- сам себя душит busy-wait'ом до 1 fps. Гейт — именно dbg_limit_fps
-- (дефолт false, строка 368; в клиенте его НИКТО больше не пишет).
--
-- Фикс: у SharedValue методы берутся через u2.__index, поэтому запись
-- ОДНОИМЁННОГО поля в сам объект их перекрывает. Ставим value=false и
-- глушим set/set_client -> серверный push молча игнорируется, busy-wait
-- недостижим. dbg_max_fps тоже пришиваем (страховка от делителя).
----------------------------------------------------------------------
local fps_locked = false
local function lock_fps_keys()
    if fps_locked then return true end
    local okAll = false
    pcall(function()
        local RS_ = game:GetService("ReplicatedStorage")
        local mod = RS_:FindFirstChild("module")
        mod = mod and mod:FindFirstChild("shared_state")
        if not (mod and mod:IsA("ModuleScript")) then return end
        local SS = require(mod).SHARED_STATE
        if type(SS) ~= "table" then return end

        local function pin(key, value)
            local sv = SS[key]
            if type(sv) ~= "table" then return false end
            sv.value = value
            sv.readonly = true
            -- перекрываем методы ИМЕННО на объекте (класс не трогаем)
            sv.set        = NOOP
            sv.set_client = NOOP
            return true
        end
        -- гейт busy-wait'а: false => внутренний while недостижим
        local a = pin("dbg_limit_fps", false)
        -- страховка: даже если гейт кто-то обойдёт, знаменатель безопасный
        local b = pin("dbg_max_fps", 240)
        okAll = a and b
    end)
    if okAll then fps_locked = true end
    return fps_locked
end
lock_fps_keys()
if not fps_locked then
    -- shared_state ещё не зареквайрен на FirstLoad — дожимаем до входа в раунд
    task.spawn(function()
        for _ = 1, 60 do
            task.wait(1)
            if lock_fps_keys() then break end
        end
    end)
end
...
-- L6: если замок не встал сейчас, его добьёт фон; сообщаем только факт
if not fps_locked then
    warn("fps-throttle lock pending (shared_state not required yet)")
end
